  ...
  if (!isFinite(value) || value === 0) {
    return (
      <span className={`inline-flex items-center gap-0.5 text-gray-500 dark:text-gray-400 ${small ? 'text-xs' : 'text-sm'}`}>
        <MinusIcon className={small ? 'w-3 h-3' : 'w-4 h-4'} />
        0%
      </span>
    );
  }
...
// Horizontal bar chart — supports negative values (center-anchored if needed)
function HBarChart({
  items,
  maxAbs,
  formatValue,
...
// Attendance bar (green/red split)
function AttendanceBar({ rate, prevRate }: { rate: number; prevRate: number }) {
  ...
  return (
    <div className="flex items-center gap-3">
      ...
      <span className={`text-xs font-semibold w-10 text-right ${colorBar === 'bg-emerald-500' ? 'text-emerald-600 dark:text-emerald-400' : colorBar === 'bg-amber-400' ? 'text-amber-600 dark:text-amber-400' : 'text-red-500 dark:text-red-400'}`}>
        {rate}%
      </span>
      {prevRate > 0 && (
...
export default function AnalyticsPage() {
  ...
  const load = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const result = await DashboardApiService.getDetailedDashboard(
        user.organizationId,
        dateFrom,
        dateTo
      );
  ...
  }, [user?.organizationId, dateFrom, dateTo, showError]);
  ...
  if (!user || !canManageUsers(user.role)) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-gray-50 dark:bg-gray-900">
        <p className="text-gray-500 dark:text-gray-400">Недостаточно прав доступа</p>
      </div>
    );
  }
  ...
  const subjectItems: { label: string; value: number; prevValue: number }[] = (
    fin?.subjectIncomeStats ?? []
  )
    .slice()
    .sort((a: SubjectIncomeStats, b: SubjectIncomeStats) => Math.abs(b.incomeAmountForPeriod) - Math.abs(a.incomeAmountForPeriod))
    .map((s: SubjectIncomeStats) => ({
      label: s.subject.subjectName,
      value: s.incomeAmountForPeriod,
      prevValue: s.incomeAmountForPreviousPeriod,
    }));

  const teacherItems: { label: string; value: number; prevValue: number }[] = (
    fin?.teacherIncomeStats ?? []
  )
    ...
    .map((t: TeacherIncomeStats) => ({
      label: t.teacher.name ?? 'Без преподавателя',
      value: t.incomeAmountForPeriod,
      prevValue: t.incomeAmountForPreviousPeriod,
    }));
  ...
  const groupAttItems: GroupAttendanceStats[] = (att?.groupAttendanceStats ?? [])
    .slice()
    .sort((a: GroupAttendanceStats, b: GroupAttendanceStats) => b.groupAttendanceRateForPeriod - a.groupAttendanceRateForPeriod);

  const teacherStatItems: TeacherStat[] = (data?.teacherStats ?? [])
    .slice()
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900 pb-12">
      {/* ── Header ── */}
      <div className="bg-gradient-to-r from-purple-600 via-purple-700 to-indigo-700 px-6 py-8 shadow-lg">
        <div className="max-w-7xl mx-auto">
          <div className="flex flex-col md:flex-row md:items-end gap-4 justify-between">
            <div>
              <h1 className="text-2xl font-bold text-white">Подробная аналитика</h1>
              {data && (
                <p className="text-purple-200 text-sm mt-1">
                  Период: {data.reportPeriodFrom} — {data.reportPeriodTo} &nbsp;·&nbsp;
            ...
            <div className="flex flex-wrap items-center gap-3">
              <div className="flex items-center gap-2 bg-white/10 backdrop-blur rounded-xl px-3 py-2">
                <span className="text-purple-200 text-xs whitespace-nowrap">С</span>
                <input
                  type="date"
                  value={dateFrom}
              ...
              <div className="flex items-center gap-2 bg-white/10 backdrop-blur rounded-xl px-3 py-2">
                <span className="text-purple-200 text-xs whitespace-nowrap">По</span>
                <input
                  type="date"
                  value={dateTo}
              ...
              <button
                onClick={load}
                disabled={loading}
                className="flex items-center gap-2 px-4 py-2 bg-white text-purple-700 font-semibold rounded-xl hover:bg-purple-50 disabled:opacity-60 transition-all text-sm shadow"
              >
          ...
          <>
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
              <KpiCard
                ...
                sub={`Пред. период: ${fmtMoney(prevIncome)}`}
                trend={incomeTrend}
            ...
            <Section title="Студенты" icon={AcademicCapIcon}>
              <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-4">
                {[
                  { label: 'Всего', val: fmt(st?.totalStudentsCount ?? 0), color: 'text-blue-600 dark:text-blue-400' },
                  { label: 'Активных', val: fmt(st?.activeStudentsCount ?? 0), color: 'text-emerald-600 dark:text-emerald-400' },
                  { label: 'Новых за период', val: fmt(st?.newStudentsForPeriodCount ?? 0), sub: `Пред: ${fmt(st?.newStudentsForPreviousPeriodCount ?? 0)}`, color: 'text-purple-600 dark:text-purple-400' },
                  { label: 'Отток', val: fmt(st?.studentsChurnForPeriodCount ?? 0), sub: `Пред: ${fmt(st?.studentsChurnForPreviousPeriodCount ?? 0)}`, color: 'text-orange-500 dark:text-orange-400' },
                  { label: '% оттока', val: `${(st?.churnForPeriodRate ?? 0).toFixed(1)}%`, sub: `Пред: ${(st?.churnForPreviousPeriodRate ?? 0).toFixed(1)}%`, color: (st?.churnForPeriodRate ?? 0) > 10 ? 'text-red-500 dark:text-red-400' : 'text-gray-900 dark:text-white' },
                ].map(item => (
                    ...
                    {groupAttItems.map((g: GroupAttendanceStats) => (
                      <div key={g.group.id} className="grid grid-cols-[1fr_auto] gap-4 items-center">
                        <div>
                          ...
                          <AttendanceBar
                            rate={g.groupAttendanceRateForPeriod}
                            prevRate={g.groupAttendanceRateForPreviousPeriod}
                          />
                        </div>
                    ...
                    {teacherStatItems.map((t: TeacherStat) => (
                      <tr key={t.teacher.id} className="hover:bg-gray-50 dark:hover:bg-gray-700/30 transition-colors">
                        ...
                        <td className="py-3 pl-3">
                          <AttendanceBar
                            rate={t.averageAttendanceRateForPeriod}
                            prevRate={t.averageAttendanceRateForPreviousPeriod}
                          />
                        </td>
                      </tr>
                    ))}
            ...
            {/* ── 6. Conversion + Retention + Low Performance ── */}
                    ...
                    {data?.lowPerformanceGroups.map(g => (
                      <div key={g.groupId} className="bg-red-50 dark:bg-red-900/20 border border-red-100 dark:border-red-800 rounded-xl p-3">
                        ...
                        <p className="text-xs text-gray-400 dark:text-gray-500 mt-0.5">
                          {g.activeStudents} активных из {g.totalStudents}
                        </p>
                      </div>
                    ))}
                  </div>
