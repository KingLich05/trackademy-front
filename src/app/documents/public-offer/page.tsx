...
export default function PublicOfferPage() {
  ...
  useEffect(() => {
    loadPublicOfferDocument();
  }, []);

  const loadPublicOfferDocument = async () => {
    try {
      setLoading(true);
      setError(null);
      ...
      // Находим публичную оферту (тип 3)
      const publicOfferDoc = documents.find(doc => doc.type === 3);
      
      if (!publicOfferDoc) {
        setError('Документ публичной оферты не найден');
        return;
      }
      ...
      console.error('Error loading public offer document:', error);
      setError('Не удалось загрузить документ публичной оферты');
    } finally {
      setLoading(false);
    }
  };
    ...
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-center max-w-md">
          ...
          <button
            onClick={loadPublicOfferDocument}
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-50 via-blue-50 to-indigo-100 dark:from-gray-900 dark:via-gray-800 dark:to-gray-900 pt-16">
      {/* Header */}
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-20">
            <div className="flex items-center space-x-6">
              ...
              >
                <ArrowLeftIcon className="h-5 w-5 text-gray-600 dark:text-gray-300 group-hover:text-blue-600 dark:group-hover:text-blue-400 transition-colors" />
              </button>
              ...
              {documentUrl && (
                <>
                  ...
                  <button
                    ...
                    className="group inline-flex items-center px-6 py-2.5 border border-transparent rounded-xl shadow-lg text-sm font-medium text-white bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-700 hover:to-indigo-700 hover:shadow-xl transition-all duration-200 hover:scale-105"
                  >
