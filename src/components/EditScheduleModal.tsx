...
const EditScheduleModal: React.FC<EditScheduleModalProps> = ({
  ...
  useEffect(() => {
    if (isOpen && schedule) {
      // Format date from "2025-10-18" to input format
  ...
  }, [isOpen, schedule, organizationId]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(error instanceof Error ? error.message : 'Произошла ошибка при обновлении шаблона расписания');
    } finally {
      setLoading(false);
    }
  };
        ...
        <div className="px-6 py-4 border-b border-gray-200 bg-indigo-50">
          <div className="flex items-center justify-between">
            <div className="flex items-center">
              ...
              <div className="ml-4">
                <h3 className="text-lg font-medium text-gray-900">Редактировать шаблон расписания</h3>
                <p className="text-sm text-gray-500">Обновите данные шаблона</p>
              </div>
            </div>
            <button
              onClick={onClose}
