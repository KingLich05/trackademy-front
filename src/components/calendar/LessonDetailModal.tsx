...
export default function LessonDetailModal({ lesson, isOpen, onClose, onUpdate }: LessonDetailModalProps) {
  ...
  const handleRestoreLesson = async () => {
    if (!restoreReason.trim()) {
      showToast('Пожалуйста, укажите причину восстановления', 'error');
      return;
    }
    ...
    try {
      ...
      showToast('Урок успешно восстановлен', 'success');
      ...
      const errorMessage = (error as { response?: { data?: { error?: string } }; message?: string })?.response?.data?.error 
        || (error as { message?: string })?.message 
        || 'Ошибка при восстановлении урока';
      showToast(errorMessage, 'error');
    } finally {
          ...
          <div className="flex gap-3">
            ...
            {activeTab === 'details' && (lesson.lessonStatus === 'Planned' || lesson.lessonStatus === 'Moved') && (isAdministrator || isTeacher || isOwner) && (
              <>
                ...
                {/* Кнопка замены преподавателя только для администратора и владельца */}
      ...
      {isRestoreModalOpen && (
        <div className="fixed inset-0 bg-gray-900/50 backdrop-blur-sm flex items-center justify-center p-4 z-[60]">
          <div className="bg-white dark:bg-gray-800 rounded-lg max-w-md w-full p-6 shadow-xl">
            ...
            <div className="mb-4">
              <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                Причина восстановления <span className="text-red-500">*</span>
              </label>
              <textarea
                value={restoreReason}
                ...
                placeholder="Укажите причину восстановления урока..."
              />
            </div>
